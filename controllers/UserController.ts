...
import { CreateUserInput, LoginUserInput, AddAddressInput, CreateCartInput } from "../dto/User.dto";
...
export const AddAddress = async (req: Request, res: Response, next: NextFunction) => {
    const { doorNo, address, lat, long } = <AddAddressInput>req.body;
    const user = req.user;
    if (user) {
...
export const AddToCart = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user) {
        ...
        if (userData) {
            var cart = await Cart.findOne({ userID: userData.id }).populate({ path: "products", populate: { path: "product", model: "food" } })
...
export const ViewCart = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user) {
        ...
        const cart = await Cart.findOne({ userID: userData.id }).populate({ path: "products", populate: { path: "product", model: "food" } })
        if (!cart) {
